...
export default function Parcours() {
  ...
  return (
    <div className="text-l lg:text-2xl flex flex-col gap-y-3 w-full my-4 z-50">
      ...
      <h5 className="text-xs font-bold mt-18 mb-16 text-center">
        Cards are clickable!
      </h5>
      <div
        id="Circle"
        className="bg-gradient-to-r from-gray-600 to-teal-100 rounded-full w-4 h-4 mx-auto"
      ></div>
      {events.map((event, key) => (
        <Fragment key={key}>
          ...
          {key < events.length - 1 && <Circle /> && <br /> && <Circle /> && (
            <Circle />
          )}
        </Fragment>
      ))}
  ...
  return (
    ...
    >
      <div className="bg-opacity-100 bg-white transition duration-200 ease-in-out trasition-transform hover:-translate-y-2  hover:shadow-2xl hover:bg-gray-200 flex flex-col gap-y-2 border shadow-md rounded-xl p-4 hover:rotate-1 cursor-pointer">
        <div
          id={heading}
          className="text-base text-blue-700 font-bold border-b"
        >
          {heading}
        </div>
